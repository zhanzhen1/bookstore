...
func AddBookCart() (handlerFunc gin.HandlerFunc) {
	return func(context *gin.Context) {
		//判断是否登录
		flag, session := dao.IsLogin(context)
		if flag {
...
// gin优化 根据用户id获取购物车信息
func GetCartInfo() (handlerFunc gin.HandlerFunc) {
	return func(context *gin.Context) {
		//判断登录
		_, session := dao.IsLogin(context)
		//获取用id
		userID := session.UserID
		//根据用户id获取对应的购物车
		cart, _ := dao.GetCartByUserID(userID)
		...
		if cart != nil {
			...
			context.HTML(http.StatusOK, "cart.html", session)
		} else {
			//该用户还没有购物车
			context.HTML(http.StatusOK, "cart.html", session)
		}
...
// 清空购物车
func DeleteCart() (handlerFunc gin.HandlerFunc) {
	return func(context *gin.Context) {
		//获取要涮菜的购物车id
		cartID := context.Query("cartID")
		fmt.Println(cartID)
...
// gin根据购物项的id删除购物项
func DeleteCartItemByID() (handlerFunc gin.HandlerFunc) {
	return func(context *gin.Context) {
		//获取要删除的购物项id
		cartItemID := context.Query("cartItemID")
...
// gin更新购物项
func UpdateCartItem() (handlerFunc gin.HandlerFunc) {
	return func(context *gin.Context) {
		//获取要删除的购物项id
		cartItemID := context.Query("cartItemID")
		bookCount := context.Query("bookCount")
		iCartItemID, _ := strconv.ParseInt(cartItemID, 10, 64)
...
//添加图书到购物车
//func AddBookCart1(w http.ResponseWriter, r *http.Request) {
//	//判断是否登录
